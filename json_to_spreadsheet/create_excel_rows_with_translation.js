...
const attachment_path = "https://idems-media-recorder.web.app/storage/project/PLH/subproject/Rapidpro/deployment/" + deployment +"/"
var lang_att = "eng"
...
for (const [file_name, flows] of Object.entries(files)) {
    ...
    let flows_sheets = {};
    let original_flows = JSON.parse(JSON.stringify(flows));
    ...
    let flows_sheets_str = JSON.stringify(flows_sheets, null, 2);
    

    const output_path = path.join(output_dir, `${file_name}.json`);
    fs.writeFile(output_path, flows_sheets_str, (err, _) => {
    ...
    if (lang){
        ...
        for (flow_name in flows_sheets){
            
            translateFlowRows(flow_name, flows_sheets[flow_name], original_flows, lang)
        }

        let flows_sheets_lang = JSON.stringify(flows_sheets, null, 2);
        const output_path_lang = path.join(lang_output_dir,`${file_name}.json`);
        fs.writeFile(output_path_lang, flows_sheets_lang, (err, _) => {
            if (err) console.log('error', err);
        });
    }
    ...
    if (curr_node.unprocessed_parents.length == 0) {
        ...
        if (uuid_processed_nodes.includes(curr_node.uuid)) {
            ...
            for (let ex = 0; ex < curr_node.exits.length; ex++) {
                ...
                if (next_node) {
                    //remove current node from list of unprocessed parents of next node
                    ...
                    if (par_index > -1) {
                        ...
                        //console.log(flow.name + ":   error, parent not in the list")
                    }
...
function createSendMsgRow(curr_node, curr_action, row_counter, from_row) {
    ...
    curr_action.attachments.forEach(file => {
        ...
        curr_row[file_type] = file_url.replace("@(fields.voiceover_audio_path & \"", attachment_path + "resourceGroup/voiceover/resourceType/audio/" + lang_att + "/")
            .replace("@(fields.relaxation_path & \"", attachment_path + "resourceGroup/relaxation/" + lang_att + "/")
            .replace("@(fields.voiceover_video_path & \"", attachment_path + "resourceGroup/voiceover/resourceType/video/" + lang_att + "/")
            .replace("@(fields.comic_path & \"", attachment_path + "resourceGroup/comic/")
            .replace("@(fields.house_of_support_path & \"", attachment_path + "resourceGroup/house_of_support/house_type/" + house_type + "/age_group/baby/family_type/normal/").replace("\" & fields.n_skills & \"", "12")
            .replace("@(fields.certificate_path & \"", attachment_path + "resourceGroup/certificate/" + lang_att + "/").replace("\" & fields.n_surveys_completed & \"", "2")
            .replace("@(fields.image_path & \"", attachment_path + "resourceGroup/image/universal/")
            .replace("@(fields.animated_audio_path & \"", attachment_path + "resourceGroup/animated/resourceType/audio/" + lang_att + "/")
            .replace("@(fields.animated_video_path & \"", attachment_path + "resourceGroup/animated/resourceType/video/" + lang_att + "/")
            .slice(0, -2);
...
function translateFlowRows(flow_name, flow_rows, original_flows, lang){
    
    let original_flow = original_flows.filter(fl => (fl.name == flow_name))[0];
    
    flow_rows.forEach((row) =>{
       
        // translate send message and quick replies
        if (row.type == "send_message"){
            
            //need to identify the corresponding action in the node
            
            let corresp_node = original_flow.nodes.filter(nd => (nd.uuid == row._nodeId) )[0];
            
            let action_uuid
            for (let ac = 0; ac < corresp_node.actions.length; ac++){
                if (corresp_node.actions[ac].type == "send_msg" && corresp_node.actions[ac].text == row.message_text ){
                    action_uuid = corresp_node.actions[ac].uuid;
                    break
                }
            }
            ...
            if (original_flow.localization[lang][action_uuid]){
                
                row.message_text = original_flow.localization[lang][action_uuid].text[0];
            
                for (let n = 1; n <=10; n++){
                    if (row["choice_" +  String(n)]){
                        row["choice_" +  String(n)] = original_flow.localization[lang][action_uuid].quick_replies[n-1];
                    }
                }
            }

            // change language for attachments
            if (row.video){
                row.video = row.video.replace("/"+ lang_att, "/" +lang)
            } else if (row.audio){
                row.audio = row.audio.replace("/"+ lang_att, "/" +lang)
            } else if (row.image && row.image.includes(lang_att)){
                row.image = row.image.replace("/"+ lang_att, "/" +lang)
            }
        ...
        if (row.condition && row.condition.length == row.from.length){
            
            for (let c=0; c< row.condition.length; c++){
                let cond = row.condition[c];
                if (cond){
                    let parent_row = flow_rows.filter(r => (r.row_id == row.from[c]))[0];
                    if (parent_row.type == "wait_for_response"){
                        let parent_node =  original_flow.nodes.filter(nd => (nd.uuid == parent_row._nodeId) )[0];
                        let case_uuid
                        for (let ca = 0; ca < parent_node.router.cases.length; ca++){
                            if (parent_node.router.cases[ca].arguments.length >0 && parent_node.router.cases[ca].arguments[0] == cond){
                                case_uuid = parent_node.router.cases[ca].uuid;
                                
                                break
                            }
                        }
    
                        if (original_flow.localization[lang][case_uuid]){
                            row.condition[c] = original_flow.localization[lang][case_uuid].arguments[0];
                        }
                    }
               }
            }
